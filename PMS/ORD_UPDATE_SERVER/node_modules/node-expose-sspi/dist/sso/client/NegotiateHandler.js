...
const debug = debug_1.default('node-expose-sspi:client');
class NegotiateHandler extends AbstractHandler_1.AbstractHandler {
    ...
    async handle(clientInfo, clientCookie, response, resource, init = {}) {
        ...
        const targetName = clientInfo.targetName === undefined
            ? await misc_1.getSPNFromURI(resource)
        ...
        while (response.headers.has('www-authenticate') &&
            ...
            if (clientSecurityContext) {
                ...
                const responseToken = msgParser_1.negotiateParse(bufferStr);
                debug('responseToken: ', responseToken);
                const buffer = misc_2.decode(bufferStr);
                input.SecBufferDesc = {
                    ulVersion: 0,
                    buffers: [buffer],
                };
            ...
            debug(misc_2.hexDump(clientSecurityContext.SecBufferDesc.buffers[0]));
            ...
            const base64 = misc_2.encode(clientSecurityContext.SecBufferDesc.buffers[0]);
            const debugObject = msgParser_1.negotiateParse(base64);
            ...
            response = await node_fetch_1.default(resource, requestInit);
