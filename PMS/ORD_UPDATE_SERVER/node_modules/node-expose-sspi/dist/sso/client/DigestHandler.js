...
class DigestHandler extends AbstractHandler_1.AbstractHandler {
    async handle(clientInfo, clientCookie, response, resource, init = {}) {
        ...
        // client nonce.
        const cnonce = misc_1.md5(Math.round(Math.random() * 1e10).toString(16)).substr(0, 16);
        ...
        const qop = 'auth';
        const digestResponse = misc_1.md5(`${ha1}:${digestChallenge.nonce}:${nonceCount}:${cnonce}:${qop}:${ha2}`);
...
function getHA2(qop, method, digestURI, entityBody) {
    if (qop === 'auth-int') {
        ...
        const a2 = `${method}:${digestURI}:${misc_1.md5(entityBody)}`;
        debug('a2: ', a2);
        return misc_1.md5(a2);
    }
    const ha2 = misc_1.md5(`${method}:${digestURI}`);
    return ha2;
}
