...
const debug = debug_1.default(DEBUG_KEY);
    ...
    return (req, res, next) => {
        ...
        (async () => {
            ...
            try {
                ...
                const buffer = misc_1.decode(token);
                ...
                if (isDbgEnabled && messageType === 'Kerberos_1') {
                    debug('Kerberos_1 details: ', kerberos_1.getKerberosDetails(buffer));
                }
                const input = {
                    credential,
                    SecBufferDesc: {
                ...
                if (isDbgEnabled && messageType.startsWith('Kerberos')) {
                    debug('Kerberos output details: ', kerberos_1.getKerberosResponseDetails(serverSecurityContext.SecBufferDesc.buffers[0]));
                }
                ...
                if (failed) {
                    ...
                    if (serverSecurityContext.SECURITY_STATUS === 'SEC_E_LOGON_DENIED') {
                        next(http_errors_1.default(401, `SEC_E_LOGON_DENIED. (incorrect login/password, or account disabled, or locked, etc.). Protocol Message = ${messageType}.`));
                        return;
                    }
                ...
                if (serverSecurityContext.SECURITY_STATUS === 'SEC_I_CONTINUE_NEEDED') {
                    ...
                    res.setHeader(WWW_AUTHENTICATE, authenticationType +
                        ' ' +
                        misc_1.encode(serverSecurityContext.SecBufferDesc.buffers[0]));
                    return res.end();
                }
                ...
                schManager.release(req);
                console.error(e);
                console.error('statusInfo: ', status_1.getStatusInfo());
                console.error('messageType: ', messageType);
                next(http_errors_1.default(401, `Error while doing SSO: ${e.message}`));
            }
        })();
    };
}
