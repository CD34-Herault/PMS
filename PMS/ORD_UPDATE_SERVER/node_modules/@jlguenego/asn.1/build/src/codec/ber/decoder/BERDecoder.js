...
const debug = debug_1.default('asn.1:BERDecoder');
...
function readSimple(cdv, identifier) {
    ...
    if (identifier.tag === TagUniversal_1.TagUniversal.BOOLEAN.code) {
        result.value = ReadBoolean_1.readBoolean(cdv, length);
        return result;
    }
    if (identifier.tag === TagUniversal_1.TagUniversal.INTEGER.code) {
        result.value = ReadInteger_1.readInteger(cdv, length);
        return result;
    }
    if (identifier.tag === TagUniversal_1.TagUniversal.BIT_STRING.code) {
        ...
        result.value = ReadBitString_1.readBitString(cdv, length);
        return result;
    }
    if (identifier.tag === TagUniversal_1.TagUniversal.OCTET_STRING.code) {
        ...
        // or constructed at the option of the sender.
        result.value = ReadOctetString_1.readOctetString(cdv, length);
        return result;
    }
    if (identifier.tag === TagUniversal_1.TagUniversal.OBJECT_IDENTIFIER.code) {
        ...
        result.value = ReadObjectIdentifier_1.readObjectIdentifier(cdv, length);
        return result;
    }
    if (identifier.tag === TagUniversal_1.TagUniversal.IA5STRING.code) {
        result.value = ReadIA5String_1.readIA5String(cdv, length);
        return result;
    }
