...
function getIdentifierOctet(m) {
    ...
    const result = misc_1.padHexString((tagClassBits + isConstructedBit + tagCode).toString(16));
    return result;
}
...
function getLengthOctet(contentLength) {
    if (contentLength < 127) {
        return misc_1.padHexString(contentLength.toString(16));
    }
    const end = misc_1.padHexString(contentLength.toString(16));
    return misc_1.padHexString((end.length / 2).toString(16)) + end;
}
...
function getContentOctet(m) {
    ...
    if (m.tagCode === TagUniversal_1.TagUniversal.INTEGER.code) {
        return misc_1.padHexString(m.value.toString(16));
    }
