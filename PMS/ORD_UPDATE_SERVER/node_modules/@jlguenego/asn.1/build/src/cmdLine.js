...
function asn1Parse() {
    commander_1.program
        ...
        .option('-f, --format <type>', "specify the message format ('bin'|'hex'|'base64')", 'bin')
    ...
    let input;
    try {
        if (commander_1.program['format'] === 'bin') {
            ...
            const b = Buffer.from(misc_1.sanitize(str), 'hex');
            input = b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength);
        }
        else if (commander_1.program['format'] === 'base64') {
            const str = fs_1.default.readFileSync(file, { encoding: 'utf8' });
            const b = Buffer.from(misc_1.sanitize(str), 'base64');
            input = b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength);
        }
        else {
    ...
    let output = ASN1_1.ASN1.parseMsg(input);
    if (asn1Definition) {
        ...
        output = module.validate(output, commander_1.program.type);
    }
    console.log(util_1.inspect(misc_1.cloneAlpha(output), false, null, true));
}
